...
async function getUserIdFromToken(token) {
    ...
    // Raw token fallback
    const data3 = await redis.get(token);
...
export async function verifyAndCredit(txHash, sessionId) {
    const redis = await getRedis();
    ...
    try {
        ...
        if (!foundValidTransfer)
            return { success: false, message: 'No valid USDC transfer to Treasury found.' };
        const usdcAmount = Number(totalUsdc) / 1_000_000;
        if (usdcAmount < MIN_PAYMENT_USDC)
            return { success: false, message: `Payment too low. Min $${MIN_PAYMENT_USDC}.` };
...
export async function getRemainingCredits(sessionId) {
    const redis = await getRedis();
    const userId = await getUserIdFromToken(sessionId);
