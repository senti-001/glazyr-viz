...
#include <memory>
#include <utility>
#include <windows.h> // Neural Chromium Shared Memory
...
void ToolbarView::Init() {
  ...
  // Neural-Chromium: Add Agent Button (Brain Icon)
  // Using MemorySaverIcon as it looks like a neural chip.
  auto neural_button = std::make_unique<ToolbarButton>(base::BindRepeating(
      [](Browser* browser, ToolbarView* view, int command, const ui::Event& event) {
        LOG(INFO) << "🧠 Neural Agent Clicked!";
        
        view->neural_agent_active_ = !view->neural_agent_active_;
        auto* button = view->neural_agent_button_.get();
        
        if (view->neural_agent_active_) {
           // Active: Highlight Blue + Start Listening
           button->SetHighlight(u"Active",  std::nullopt); // uses default highlight color
           view->InitNeuralInput();
           view->input_timer_.Start(FROM_HERE, base::Milliseconds(100), base::BindRepeating(&ToolbarView::OnNeuralInputTimer, base::Unretained(view)));
        } else {
           // Inactive: Clear Highlight + Stop Listening
           button->SetHighlight(u"", std::nullopt);
           view->input_timer_.Stop();
           view->location_bar_->GetOmniboxView()->SetUserText(u""); // Optional: Clear text?
        }
      },
      browser_, this, 0));
  // neural_button->SetVectorIcon(vector_icons::kExtensionIcon); // Removed Puzzle Piece
  neural_button->SetText(u" 🧠 ");
  neural_button->SetTooltipText(u"Neural Agent (Brain Mode)");
  neural_button->SetVisible(true);
  neural_agent_button_ = AddChildView(std::move(neural_button));
  ...
  // Neural-Chromium: Initialize STT Return Path
  InitNeuralInput();
  input_timer_.Start(FROM_HERE, base::Milliseconds(100),
                     this, &ToolbarView::OnNeuralInputTimer);
...
void ToolbarView::InitNeuralInput() {
  if (input_shm_handle_) return; // Already initialized

  // Create/Open Shared Memory "NeuralChromium_Input_Text"
  // 4KB size
  HANDLE hMapFile = CreateFileMappingW(
      ...
      L"NeuralChromium_Input_Text"); // name of mapping object
  ...
  input_shm_handle_ = hMapFile;
  input_shm_view_ = MapViewOfFile(hMapFile, FILE_MAP_ALL_ACCESS, 0, 0, 4096);
  
  if (input_shm_view_) {
     // Clear it initially
     memset(input_shm_view_, 0, 4096);
  }
}

void ToolbarView::OnNeuralInputTimer() {
   if (!input_shm_view_ || !location_bar_) return;
   
   // Protocol:
   // Byte 0-3: Revision (uint32)
   // Byte 4-7: Length (uint32)
   // Byte 8...: UTF-8 Text
   
   uint32_t* revision_ptr = static_cast<uint32_t*>(input_shm_view_);
   static uint32_t last_revision = 0;
   
   uint32_t current_revision = *revision_ptr;
   if (current_revision != last_revision && current_revision > 0) {
       last_revision = current_revision;
       
       uint32_t length = *(revision_ptr + 1);
       if (length > 0 && length < 1000) {
           // Only update if Omnibox is NOT being edited (user is not typing)
           OmniboxView* omnibox = location_bar_->GetOmniboxView();
           if (omnibox && !omnibox->IsEditingOrEmpty()) {
               char* text_ptr = reinterpret_cast<char*>(revision_ptr + 2);
               std::string utf8_text(text_ptr, length);
               std::u16string utf16_text = base::UTF8ToUTF16(utf8_text);
               
               omnibox->SetUserText(utf16_text);
           }
       }
   }
}
