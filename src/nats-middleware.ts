import { StringCodec, Msg, JsMsg } from 'nats';
 ...
 */
export async function withZeroTrustNats(
    msg: Msg,
    executeVizCommand: (payload: any, userId: string) => Promise<void>
) {
    // Cast to JsMsg to access JetStream-specific methods like ack() and term()
    const jm = msg as unknown as JsMsg;
    ...
    try {
        ...
        const sessionData = await redis.get<{ userId: string; expires: string }>(sessionKey);
        ...
        if (typeof jm.ack === 'function') jm.ack();
        ...
        if (typeof jm.term === 'function') jm.term();
    }
}
