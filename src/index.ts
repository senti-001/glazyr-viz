...
let localRedisClient: ReturnType<typeof createClient>;
 ...
 */
export const createServer = (sessionId: string = "default-session", getSessionCount?: () => number) => {
    ...
    async function fetchOpticNerveStatus() {
        if (!isRedisConnected) {
            return {
                content: [{ type: "text" as const, text: JSON.stringify({ status: "OFFLINE", reason: "Redis Tunnel not detected at 127.0.0.1:6379" }, null, 2) }]
            };
        }
        try {
            ...
            if (!rawFrame) {
                return {
                    content: [{ type: "text" as const, text: "### [Glazyr Viz] System IDLE\n\nWaiting for telemetry stream from GCP..." }]
                };
            }
            ...
            return {
                content: [{ type: "text" as const, text: `### [Glazyr Viz] Telemetry Error\n\nError accessing Optic Nerve: ${e.message}` }],
                isError: true
            };
        }
    }
    ...
    server.tool(
        ...
        async () => {
            ...
            if (!(await consumeCredit(sessionId))) {
                ...
                return {
                    content: [{ type: "text" as const, text: `⚠️ [Glazyr Viz] Credit Exhausted. Please top up your session. Balance: ${balance.toLocaleString()} credits.` }],
                    isError: true
                };
            }
            const balance = await getRemainingCredits(sessionId);
            ...
            try {
                const text = (status.content[0] as any).text;
                ...
                if (match) {
                    ...
                    status.content[0] = { type: "text" as const, text: updatedText };
                }
            } catch (e) {
    ...
    server.tool(
        ...
        async ({ tx_hash }) => {
            const result = await verifyAndCredit(tx_hash as `0x${string}`, sessionId);
            return {
                content: [{ type: "text" as const, text: result.success ? `✅ Payment Verified! Granted ${result.grantedCredits} frames.` : `❌ ${result.message}` }]
            };
        }
    );
    ...
    server.tool(
        ...
        async () => {
            const balance = await getRemainingCredits(sessionId);
            return {
                content: [{ type: "text" as const, text: `🧠 Vision Balance: ${balance.toLocaleString()} cognitive frames remaining.` }]
            };
        }
    );
    ...
    server.tool(
        "peek_vision_buffer",
        "Captures a single high-resolution raw frame from the GPU-accelerated vision buffer.",
        {
        ...
        async ({ include_base64 }) => {
            ...
            if (!(await consumeCredit(sessionId))) {
                return { content: [{ type: "text" as const, text: "⚠️ Credit Exhausted." }], isError: true };
            }
            try {
                if (fs.existsSync(SHM_PATH)) {
                    ...
                    if (rawBuf.length >= 32 && rawBuf.readUInt32LE(0) === MRCN_MAGIC) {
                        ...
                        return { content: [{ type: "text" as const, text: JSON.stringify(visionData, null, 2) }] };
                    }
                }

                // Fallback to Redis
                if (!isRedisConnected) {
                    return { content: [{ type: "text" as const, text: JSON.stringify({ status: "OFFLINE", reason: "Redis Tunnel not detected at 127.0.0.1:6379" }, null, 2) }] };
                }
                ...
                if (rawFrame) {
                    ...
                    return { content: [{ type: "text" as const, text: JSON.stringify(frame, null, 2) }] };
                }

                return { content: [{ type: "text" as const, text: "No vision buffer available (SHM or Redis)." }], isError: true };
            } catch (err: any) {
                return { content: [{ type: "text" as const, text: `Error peeking vision buffer: ${err.message}` }], isError: true };
            }
        }
    );
    ...
    server.tool(
        ...
        async ({ url }) => {
            if (!isLocalRedisConnected) {
                return { content: [{ type: "text" as const, text: JSON.stringify({ status: "OFFLINE", reason: "Local Redis not connected for CDP commands" }, null, 2) }] };
            }
            if (!(await consumeCredit(sessionId))) {
                return { content: [{ type: "text" as const, text: "⚠️ Credit Exhausted." }], isError: true };
            }
            ...
            const balance = await getRemainingCredits(sessionId);
            return { content: [{ type: "text" as const, text: `✅ Navigate command dispatched to ${url}. Balance: ${balance.toLocaleString()} credits.` }] };
        }
    );
    ...
    server.tool(
        ...
        async ({ count }) => {
            ...
            const balance = await getRemainingCredits(sessionId);
            return { content: [{ type: "text" as const, text: `✅ Command dispatched: Set fish count → ${count}. Balance: ${balance.toLocaleString()} credits.` }] };
        }
    );

    server.tool(
        "evaluate_js",
        "Evaluates arbitrary JavaScript in the GCP GCP Perception Node browser. Use this to inspect complex WebGL states or trigger custom simulation events.",
        {
        ...
        async ({ script }) => {
            ...
            for (let i = 0; i < 150; i++) {
                ...
                if (reply) {
                    ...
                    return { content: [{ type: "text" as const, text: `✅ JS Result: ${reply}` }] };
                }
                await new Promise(resolve => setTimeout(resolve, 100));
            }

            return { content: [{ type: "text" as const, text: "✅ JS command dispatched (no reply received within 15s)" }] };
        }
    );

    server.tool(
        "run_dogfood_surge",
        "Executes a standardized dogfooding sequence: sets baseline, triggers a 30,000 fish surge, and returns visual health telemetry.",
        {},
        async () => {
    ...
    server.prompt(
        "vision-auditor",
        "Instructs the agent to perform a comprehensive visual health check of the GCP GCP Perception Node cluster.",
        {
    ...
    server.tool("key", { key: z.string() }, async ({ key }) => {
        ...
        const balance = await getRemainingCredits(sessionId);
        return { content: [{ type: "text" as const, text: `✅ Key command dispatched: ${key}. Balance: ${balance.toLocaleString()} credits.` }] };
    });
